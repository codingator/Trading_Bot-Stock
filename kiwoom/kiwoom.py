...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.screen_calculations_stock = "4000"
        ...
        self.dynamicCall("SetRealReg(QString, QString, QString, QString)", self.screen_start_stop_real, '', self.realType.REALTYPE['장시작시간']['장운영구분'], '0')  # 추가 값 받으려면 1로 변경
    ...
    # API 데이터 전송 후, 받을 데이터 대기 이벤트(로그인, 거래, 기타)
    def event_slots(self):
        self.OnEventConnect.connect(self.login_slot)
        self.OnReceiveTrData.connect(self.trdata_slot)
        self.OnReceiveMsg.connect(self.msg_slot)

    # 실시간 데이터 전소 후, 받을 데이터 대기 이벤트(종목, 계좌잔고?)
    def real_event_slots(self):
        self.OnReceiveRealData.connect(self.real_data_slot)
        self.OnReceiveChejanData.connect(self.chejan_slot)

    # API 로그인, 이벤트 루프 생성 및 실행
    ...
    def get_account_info(self):
        ...
        print("나의 보유 계좌번호 %s" % self.account_num)  # 8138544511
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "계좌평가잔고내역요청":
            ...
            # GetRepeatCnt는 1회 20개만 조회됨(sPrevNext = 2로 다음 데이터 호출)
            ...
            for i in range(rows):
                ...
                dividend_rate = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                 "수익률(%)")
                ...
                define_acc = self.account_stock_dict[code]

                # {"02039": {"종목명": "어디", "보유수량": 1, ...}}
                define_acc.update({"종목명": code_name})
                define_acc.update({"보유수량": stock_quantity})
                define_acc.update({"매입가": buy_price})
                define_acc.update({"수익률(%)": dividend_rate})
                define_acc.update({"현재가": current_price})
                define_acc.update({"매입금액": total_chegual_price})
                define_acc.update({"매매가능수량": possible_quantity})

                cnt += 1

            print("계좌 보유 종목 : %s" % self.account_stock_dict)
            print("계좌 보유종목 수 : %s" % cnt)

            # 다음 페이지 유무에 따라 유: "1", 무: "0" or ""으로 반환되어 옴
            if sPrevNext == "2":
            ...
            for i in range(rows):
                ...
                order_price = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                               "주문가격")
                # -매도, +매수, 정정/취소
                order_division = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i,
                                                  "주문구분")
                ...
                define_acc = self.not_account_stock_dict[order_no]

                define_acc.update({"종목코드": code})
                define_acc.update({"종목명": code_name})
                define_acc.update({"주문번호": order_no})
                define_acc.update({"주문상태": order_status})
                define_acc.update({"주문수량": order_quantity})
                define_acc.update({"주문가격": order_price})
                define_acc.update({"주문구분": order_division})
                define_acc.update({"미체결수량": not_quantity})
                define_acc.update({"체결량": ok_quantity})

                print("미체결 종목 : %s" % define_acc)
            ...
            if sPrevNext == "2":
                ...
                if self.calcul_data == None or len(self.calcul_data) < 120:
                    ...
                    if int(self.calcul_data[0][7]) <= moving_average_price and int(
                            self.calcul_data[0][6]) <= moving_average_price:
                    ...
                    if bottom_stock_price == True:
                        ...
                        while True:
                            if len(self.calcul_data[idx:]) < 120:  # 120일치 유무 확인
                                print("120일치 없음!")
                                break
                        ...
                        if price_top_moving == True:
                            if moving_average_price > moving_average_price_prev and check_price > prev_price:
                                print("포착된 이평선의 가격이 오늘자(최근 일자) 이평선 가격보다 낮은 것 확인 됨")
                                print("포탁된 부분의 일봉 저가가 오늘자 일봉의 고가보다 낮은지 확인 됨")
                                pass_success = True
    ...
    def calculator_fnc(self):
        ...
        for idx, code in enumerate(code_list):
            self.dynamicCall("DisconnectRealData(QString)", self.screen_calculations_stock)
    ...
    def day_kiwoom_db(self, code=None, date=None, sPrevNext="0"):
        ...
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "주식일봉차트조회", "opt10081", sPrevNext,
                         self.screen_calculations_stock)
    ...
    def screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            if code == self.portfolio_stock_dict.keys():
                self.portfolio_stock_dict[code].update({"스크린번호": str(self.screen_real_stock)})
                self.portfolio_stock_dict[code].update({"주문용스크린번호": str(self.screen_meme_stock)})

            elif code != self.portfolio_stock_dict.keys():
    ...
    def real_data_slot(self, sCode, sRealType, sRealData):
        if sRealType == '장시작시간':
            ...
            c = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['체결시간'])  # 출력: +(-)50
            c = abs(int(c))

            d = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['등락율']) # 출력: +(-)12.5
            d = float(d)
            ...
            g = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['거래량'])  # 출력: +240124 매수일 때, -2034 매도일 때
            g = abs(int(g))
            ...
            # 오늘 산 잔고에 있을 경우
                ...
                if jd['주문단가수량'] > 0 and (meme_rate > 5 or meme_rate < -5):
            ...
            # 실시간으로 데이터 바뀌면 진행중이던 루프에서 오류 생김
            not_meme_list = list(self.not_account_stock_dict)
            for order_num in not_meme_list:
                code = self.not_account_stock_dict[order_num]['종목코드']
                meme_price = self.not_account_stock_dict[order_num]['주문가격']
                not_quantify = self.not_account_stock_dict[order_num]['미체결수량']
                # meme_division = self.not_account_stock_dict[order_num]['매도수구분']
                order_division = self.not_account_stock_dict[order_num]['주문구분']

                if order_division == '매수' and not_quantify > 0 and e > meme_price:
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFIdList):
        if sGubun == '0':
            ...
            origin_order_number = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['원주문번호'])  # 출력: default: '000000'
            ...
            order_division = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['주문구분'])  # 출력: -매도, +매수
            order_division = order_division.strip().lstrip('+').lstrip('-')

            # meme_division = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['매도수구분'])  # 출력: 매수: 2, 매도: 1
            # meme_division = self.realType.REALTYPE['매도수구분'][meme_division]
            ...
            first_sell_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['(최우선)매도호가'])  # 출력: -6001
            ...
            self.not_account_stock_dict[order_number].update({'주문구분': order_division})
            #self.not_account_stock_dict[order_number].update({'매도수구분': meme_division})
            ...
            total_buy_price = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['총매입가'])  # 계좌에 있는 종목
            total_buy_price = int(total_buy_price)

            meme_division = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['잔고']['매도매수구분'])
            meme_division = self.realType.REALTYPE['매도수구분'][meme_division]
            ...
            self.jango_dict[sCode].update({'매도매수구분': meme_division})
