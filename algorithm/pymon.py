...
import time
import datetime
import dataScraping as webreader
import numpy as np
from pandas import DataFrame
from PyQt5.QtWidgets import *
import kiwoom.kiwoom_test
...
class PyMon:
    def __init__(self):
        self.kiwoom = kiwoom.kiwoom_test.Kiwoom()
    ...
    def calculate_estimated_dividend_to_treasury(self, code):
        ...
        estimated_dividend_to_treasury = float(estimated_dividend_yield.replace(',', '')) / float(current_3year_treasury.replace(',', ''))
    ...
    def get_min_max_dividend_to_treasury(self, code):
        ...
        for year in range(cur_year - 5, cur_year):
            if year in previous_dividend_yield.keys() and year in three_years_treasury.keys():
                ratio = float(previous_dividend_yield[year].replace(',','')) / float(three_years_treasury[year].replace(',',''))
    ...
    def buy_check_by_dividend_algorithm(self, code):
        ...
        # 1 == 매수, 0 == 매도
        if estimated_dividend_to_treasury >= max_ratio != 0:
